...
class OrderFunctionalityTests : BaseTest() {
    ...
    fun `radio_buttons_update_entree_menu_subtotal`() {
        ...
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "7.00"))))

        // Select the chili item
        onView(withId(R.id.chili)).perform(click())
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "4.00"))))

        // Select the pasta item
        onView(withId(R.id.pasta)).perform(click())
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "5.50"))))
    ...
    fun `radio_buttons_update_side_menu_subtotal`() {
        ...
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "2.50"))))

        // Select the soup item
        onView(withId(R.id.soup)).perform(click())
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "3.00"))))

        // Select the potato item
        onView(withId(R.id.potatoes)).perform(click())
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "2.00"))))

        // Select the rice item
        onView(withId(R.id.rice)).perform(click())
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "1.50"))))
    }
    ...
    fun `radio_buttons_update_accompaniment_menu_subtotal`() {
        ...
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "0.50"))))

        // Select the soup item
        onView(withId(R.id.berries)).perform(click())
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "1.00"))))
    ...
    fun `subtotal_updates_in_full_order_flow`() {
        ...
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "9.50"))))
        // Move to next fragment
        onView(withId(R.id.next_button)).perform(click())
        ...
        // Check that subtotal has updated
        onView(withId(R.id.subtotal))
            .check(matches(withText(containsString("Subtotal: " + pricePrefix + "10.00"))))
        // Move to next fragment
        onView(withId(R.id.next_button)).perform(click())
    ...
    fun `subtotal_tax_total_in_checkout`() {
        ...
        onView(withId(R.id.tax))
            .check(matches(withText(containsString("Tax: " + pricePrefix + "0.80"))))
        // Check total
        onView(withId(R.id.total))
            .check(matches(withText(containsString("Total: " + pricePrefix + "10.80"))))
    }
    ...
    fun `order_reset_after_cancel_from_entree_menu`() {
        ...
        onView(withText("Subtotal: " + pricePrefix + "0.00")).check(matches(isDisplayed()))
    }
